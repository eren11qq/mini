...
- ★ 真机裁决三轮(2026-09-16,用户验后):① 纯反色带太亮 → 透明轻档;② 透明档不直观 → 毛玻璃近似压暗带(7m+2m);③ **终档 = 零背景,选中 = `/命令` 本体换主题淡蓝(`\x1b[36m\x1b[1m`)+ ● CYAN 位标,desc 恒灰不变**。`INV` 叶再撤,整层零 `\x1b[7m` 由测试反向钉死;折行/视口/无匹配/≤height 契约全程未动,未选行 diff=0 锚三轮皆成立。
