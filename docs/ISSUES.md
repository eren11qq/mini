...
- ★ 真机裁决两轮(2026-09-16,用户验后):① 纯反色带太亮 → 改透明轻档;② 透明档不直观、要「毛玻璃」背景 → 定档 **压暗带 `\x1b[7m\x1b[2m\x1b[1m`**(反色+DIM=深灰柔带,pad 满宽在带内,逐物理行 RESET 自闭,● 位标保留)。ANSI 无真 alpha,DIM+INV = 终端内最近毛玻璃;`INV` 叶回归。折行/视口/无匹配/≤height 契约不变,非选中行恒零 `\x1b[7m`。
