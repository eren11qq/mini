...
| H2  | flags 全集仅 3 个:--model / --continue / --resume(编号选择器);会话内斜杠命令 2 条:/model \<alias\> 热切(本会话裁决)+ /compact(H3);厂商只是配置行(providers.ts),--model 与 model_change 均存 alias                                          |
| H3  | systemPrompt = 中文骨架 + 激活工具清单 + 项目上下文(AGENTS.md/CLAUDE.md 向上找、跨目录近者赢、同目录 AGENTS 赢);工具集变化即重建(每轮纯函数重算);/compact = force 手动压 + 每轮 message_end 后自动阈值门,手动/自动共用同厂商流 summarizeFn |
